document.addEventListener('DOMContentLoaded', () => {
    ...
    const sumTwo = (arr) => {
        for (let i = 0; i < arr.length - 1; i++) {
            let numOne = 2020 - arr[i];
            if (arr.includes(numOne)) {
                let indexNumOne = arr.indexOf(numOne)
                return (arr[i] * arr[indexNumOne])
            }
        }
    }
    ...
    const sumThree = (arr) => {
        ...
        for (let i = 0; i < arr.length; i++) {
            for (let j = i + 1; j < arr.length; j++) {
                let sumOfTwo = arr[i] + arr[j]
                let lastNum = 2020 - sumOfTwo;
                if (arr.includes(lastNum)) {
                    let indexLastNum = arr.indexOf(lastNum);
                    //console.log('value ',arr[indexLastNum])
                    return arr[i] * arr[j] * arr[indexLastNum]
    ...
    console.log(sumTwo(input));
    console.log(sumThree(input));
...
// sumThree(input); // Whatever is timed goes between the two "console.time"

// console.timeEnd('sumTwo')