...
const validPassword1 = (db) => {

    let validPassword = 0;

    for (let i = 0; i < db.length; i++) {
        const splitPart = db[i].split(': ');
        const numAndStr = splitPart[0].split(' ')
        const minMax = numAndStr[0].split('-')
        const count = splitPart[1].split(numAndStr[1]).length - 1; // find str.length in strings 

        // console.log('SplitPart: ', splitPart)
        // console.log('Number and strings:', numAndStr)
        //  console.log('MinMax: ', minMax)
        //  console.log('Str: ', numAndStr[1])
        //  console.log('strings: ', splitPart[1])

        if (splitPart[1].includes(numAndStr[1]) && count >= minMax[0] && count <= minMax[1]) {
            validPassword++;
...
const validPassword2 = (db) => {

    let validPassword = 0;

    for (let i = 0; i < db.length; i++) {
        let splitPart = db[i].split(': ');
        let numAndStr = splitPart[0].split(' ')
        ...
        // console.log('Number and strings:', numAndStr)
        // console.log('MinMax: ', minMax)
        // console.log(splitPart[1][Number(minMax[0]) - 1]);
        // console.log(splitPart[1][Number(minMax[1]) - 1]);

        // only one position that can cantain the same string as provided
        if (!(splitPart[1][Number(minMax[0]) - 1] === numAndStr[1] && splitPart[1][Number(minMax[1]) - 1] === numAndStr[1])) {
            if ((splitPart[1][Number(minMax[0]) - 1] === numAndStr[1] || splitPart[1][Number(minMax[1]) - 1] === numAndStr[1])) {
                validPassword++
            }
        }
...
console.log(validPassword1(passwordList))
console.log(validPassword2(passwordList))
